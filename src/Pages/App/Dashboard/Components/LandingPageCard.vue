<template>
    <div class="card card-bordered border border-gray-300">
        <figure>
            <img :src="`${baseUrl}images/showcase.jpg`" loading="lazy">
        </figure> 
        <div class="card-body p-4">
            <h2 class="card-title text-base font-normal m-0 p-0">{{ landingPage.name }}</h2> 
            <div class="justify-end card-actions m-0 p-0">
                <router-link :to="'/' + landingPage.slug" target="_blank" class="btn btn-square btn-sm btn-outline btn-primary">
...
<script>
    ...
    export default defineComponent({
        ...
        setup(props) {
            ...
            return { remove, removing, baseUrl }
        },
    })
</script>
